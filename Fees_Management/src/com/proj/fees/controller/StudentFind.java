...
public class StudentFind extends JFrame {
		...
		find.addActionListener(e -> {
			...
			if (r == null) {
				...
				if (cnt == 0) {
					...
					firstDateOfFeesSub = (firstDateOfFeesSub + "-" + String.valueOf(submissionDate.get(Calendar.YEAR)))
							.replace("-", "/");
	...
	public Registration getRegisteredObjectWithAmt(Calendar firstDate, Calendar submissionDate, Registration r) {
		...
		subDate = String.valueOf(
				submissionDate.get(Calendar.DATE) + "/" + (new SimpleDateFormat("MMM").format(submissionDate.getTime()))
						+ "/" + submissionDate.get(Calendar.YEAR));
		dDate = String.valueOf(dueDate.get(Calendar.DATE) + "/"
				+ (new SimpleDateFormat("MMM").format(dueDate.getTime())) + "/" + dueDate.get(Calendar.YEAR));
